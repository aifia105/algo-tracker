...
const Card = ({ filteredProblems }: CardProps) => {
  const [toggleNoteDialog, setToggleNoteDialog] = useState(false);
    ...
    <>
      {toggleNoteDialog && (
        <Notes
          ...
          setToggleNoteDialog={setToggleNoteDialog}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProblems.map((problem, index) => (
          <div
            key={
              problem.id || `${problem.problemId}-${problem.userId}-${index}`
            }
          ...
          >
            ...
            <div className="border-t border-custom pt-4 flex items-center justify-between gap-6">
              ...
              <button
                onClick={() => handleNoteClick(problem.notes || "")}
                className="text-secondary text-sm cursor-pointer underline"
              >
                Notes
              </button>
            </div>
