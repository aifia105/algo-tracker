...
export const getProblemsTags = async (token: string) => {
  try {
    const response = await fetch(`${BASE_URL}/api/problems/tags`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      throw new Error("Failed to fetch problems tags");
    }

    return await response.json();
  } catch (error) {
    console.error("Error fetching problems tags:", error);
    throw error;
  }
};
